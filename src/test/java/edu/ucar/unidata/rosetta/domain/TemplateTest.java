...
public class TemplateTest {
  ...
  private void createBogusTemplateModifier() {
    ...
    List<RosettaGlobalAttribute> globalMetadata = new ArrayList<>();
    modifiedGlobalAttrUpdated =
        new RosettaGlobalAttribute(modifiedGlobalAttrOld.getName(), "yo-updated", modifiedGlobalAttrOld.getType());
  ...
  private void readWavegliderTemplate() throws IOException {
    File wavegliderTemplateFile =
        Paths.get(TestUtils.getTestDataDirStr(), "singleTrajectory", "waveglider", "rosetta.template").toFile();
  ...
  public void testBogusModifiedGeneral() throws IOException {
    ...
    // finally, let's make sure that the template updated (not a test for correctness of update, just that it was
    // modified)
