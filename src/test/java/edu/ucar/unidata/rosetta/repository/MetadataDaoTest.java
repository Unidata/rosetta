...
import edu.ucar.unidata.rosetta.repository.wizard.JdbcMetadataDao;
...
public class MetadataDaoTest {
    ...
    public void setUp() throws Exception {
        ...
        when(metadataDao.lookupMetadata("000000345HDV4")).thenReturn(Arrays.asList(metadata1, metadata2));
        when(metadataDao.lookupMetadata("000000345HDV4", "general")).thenReturn(Arrays.asList(metadata1));
        doThrow(new DataRetrievalFailureException("Unable to delete metadata entries corresponding to id " + metadata1.getId())).when(metadataDao).deletePersistedMetadata(metadata1.getId());
        doThrow(new DataRetrievalFailureException("Unable to delete metadata entries corresponding to id " + metadata1.getId() + " and type " + metadata1.getType())).when(metadataDao).deletePersistedMetadata(metadata1.getId(), metadata1.getType());
    }
