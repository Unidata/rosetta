...
public class TemplateManagerImpl implements TemplateManager {
  ...
  public Template createTemplate(String id) throws RosettaFileException {
    ...
    if (!format.equals("etuff")) {
      ...
      for (Variable variable : variables) {
        ...
        if (name.equals("do_not_use") || name.equals("DO_NOT_USE")) {
          ...
          // variableMetadata
          List<RosettaAttribute> variableMetadata =
              populateVariableData(variable.getRequiredMetadata(), metadataProfiles);
    ...
    for (GlobalMetadata item : globalMetadata) {
      if (item.getMetadataValueType() != null) {
        rosettaGlobalAttributes.add(new RosettaGlobalAttribute(item.getMetadataKey(), item.getMetadataValue(),
            item.getMetadataValueType().toUpperCase(), item.getMetadataGroup()));
      } else {
        rosettaGlobalAttributes.add(new RosettaGlobalAttribute(item.getMetadataKey(), item.getMetadataValue(), "STRING",
            item.getMetadataGroup()));
      }
    }
   ...
   */
  private List<RosettaAttribute> populateVariableData(List<VariableMetadata> requiredMetadata,
      List<MetadataProfile> metadataProfiles) {
  ...
  private void writeTemplateToFile(Template template, String id) throws RosettaFileException, IOException {
    ...
    try (BufferedWriter bufferedWriter = new BufferedWriter(new FileWriter(new File(templateFilePath)))) {
