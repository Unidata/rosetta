...
public class JdbcGlobalMetadataDao extends JdbcDaoSupport implements GlobalMetadataDao {
  ...
  public List<GlobalMetadata> lookupGlobalMetadata(String wizardDataId) {
    ...
    List<GlobalMetadata> globalMetadata =
        getJdbcTemplate().query(sql, new JdbcGlobalMetadataDao.GlobalMetadataMapper(), wizardDataId);
  ...
  public void updatePersistedGlobalMetadata(String wizardDataId, GlobalMetadata globalMetadata)
    ...
    String sql = "UPDATE globalMetadata SET " + "wizardDataId = ?, " + "metadataGroup = ?, " + "metadataValueType = ?, "
        + "metadataKey = ?, " + "metadataValue = ? " + "WHERE wizardDataId = ? " + "AND metadataGroup = ? "
        + "AND metadataKey = ? ";

    int rowsAffected = getJdbcTemplate().update(sql, new Object[] {
        // order matters here
        globalMetadata.getWizardDataId(), globalMetadata.getMetadataGroup(), globalMetadata.getMetadataValueType(),
        globalMetadata.getMetadataKey(), globalMetadata.getMetadataValue(), globalMetadata.getWizardDataId(),
        globalMetadata.getMetadataGroup(), globalMetadata.getMetadataKey()});
    if (rowsAffected <= 0) {
  ...
  public void deletePersistedGlobalMetadata(GlobalMetadata globalMetadata) throws DataRetrievalFailureException {
    ...
    if (rowsAffected <= 0) {
      String message = "Unable to delete global metadata corresponding to id " + globalMetadata.getWizardDataId();
      logger.error(message);
      throw new DataRetrievalFailureException(message);
    } else {
