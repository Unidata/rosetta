...
public class JdbcPlatformResourceDao extends JdbcDaoSupport implements PlatformResourceDao {
  ...
  public List<Platform> getPlatforms() throws DataAccessException {
    String sql =
        "SELECT platforms.id, platforms.name, platforms.imgPath, cfTypes.name, communities.name FROM platforms JOIN communities ON platforms.community = communities.id JOIN cfTypes ON platforms.cfType = cfTypes.id";
