...
public class JdbcUserDao extends JdbcDaoSupport implements UserDao {
    ...
    public void updatePassword(User user) throws DataAccessException {
        ...
        if (rowsAffected  <= 0) {
            String message = "Unable to update user's password.  User with user name " + user.getUserName() + " not found.";
            logger.error(message);
            throw new DataRetrievalFailureException(message);
        } else {
    ...
    public User updateUser(User user) throws DataAccessException {
        ...
        if (rowsAffected  <= 0) {
            String message ="Unable to update User.  No user with user name " + user.getUserName() + " found.";
            logger.error(message);
            throw new DataRetrievalFailureException(message);
        } else {
