...
public class JdbcVariableDao extends JdbcDaoSupport implements VariableDao {
  ...
  public List<Variable> lookupVariables(String wizardDataId) {
    ...
    for (Variable variable : variables) {
      List<VariableMetadata> variableMetadataValues =
          getJdbcTemplate().query(sql, new JdbcVariableDao.VariableMetadataMapper(), variable.getVariableId());
  ...
  public int persistVariable(String wizardDataId, Variable variable) throws DataRetrievalFailureException {
    ...
    this.insertActor =
        new SimpleJdbcInsert(getDataSource()).withTableName("variables").usingGeneratedKeyColumns("variableId");
  ...
  private void persistVariableMetadata(int variableId, List<VariableMetadata> variableMetadataValues,
      String complianceLevel) throws DataRetrievalFailureException {
    ...
    for (VariableMetadata variableMetadata : variableMetadataValues) {
      ...
      if (rowsAffected <= 0) {
        ...
        logger.info("Persisted variable metadata value for " + variableMetadata.getMetadataKey()
            + " corresponding to variable " + variableId);
      }
    }
  }
  ...
  public void updatePersistedVariable(Variable variable) throws DataRetrievalFailureException {
    String sql = "UPDATE variables SET " + "wizardDataId = ?, " + "columnNumber = ?, " + "variableName = ?, "
        + "metadataType = ?, " + "metadataTypeStructure = ?, " + "verticalDirection = ?, " + "metadataValueType = ? "
        + "WHERE variableId = ?";
    int rowsAffected = getJdbcTemplate().update(sql, new Object[] {
        // order matters here
        variable.getWizardDataId(), variable.getColumnNumber(), variable.getVariableName(), variable.getMetadataType(),
        variable.getMetadataTypeStructure(), variable.getVerticalDirection(), variable.getMetadataValueType(),
  ...
  private void deletePersistedVariableMetadata(int variableId) {
    ...
    List<VariableMetadata> variableMetadataValues =
        getJdbcTemplate().query(sql, new JdbcVariableDao.VariableMetadataMapper(), variableId);
  ...
  public void deletePersistedVariable(Variable variable) throws DataRetrievalFailureException {
    ...
    if (rowsAffected <= 0) {
      String message = "Unable to delete variable corresponding to id " + variable.getVariableId();
      logger.error(message);
      throw new DataRetrievalFailureException(message);
    } else {
