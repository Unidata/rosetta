...
public class JdbcWizardDataDao extends JdbcDaoSupport implements WizardDataDao {
  ...
  public void persistWizardData(WizardData wizardData) throws DataRetrievalFailureException {
    ...
    String sql = "SELECT * FROM wizardData WHERE id = ?";
    List<WizardData> persisted = getJdbcTemplate().query(sql, new WizardDataDataMapper(), wizardData.getId());
  ...
  public void updatePersistedWizardData(WizardData wizardData) throws DataRetrievalFailureException {
    String sql = "UPDATE wizardData SET " + "cfType = ?, " + "community = ?, " + "metadataProfile = ?,"
        + "platform = ?, " + "headerLineNumbers = ?, " + "delimiter = ?, " + "dataFileType = ? " + "WHERE id = ?";
    int rowsAffected = getJdbcTemplate().update(sql, new Object[] {
        // order matters here
        wizardData.getCfType(), wizardData.getCommunity(), wizardData.getMetadataProfile(), wizardData.getPlatform(),
        wizardData.getHeaderLineNumbers(), wizardData.getDelimiter(), wizardData.getDataFileType(),
        wizardData.getId()});
    if (rowsAffected <= 0) {
      String message = "Unable to update persisted wizard data corresponding to id " + wizardData.getId();
      logger.error(message);
      throw new DataRetrievalFailureException(message);
    } else {
