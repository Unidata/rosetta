  ...
  var $event = $.event,
      ...
      drag = $special.drag = {
        ...
        init: function (event) {
          ...
          if (dd.handle && !$(event.target).closest(dd.handle,
        ...
        interaction: function (elem, dd) {
          var offset = (elem && elem.ownerDocument)
              ? $(elem)[dd.relative ? "position" : "offset"]() || {
            top: 0,
            left: 0
          }
              : {top: 0, left: 0};
        ...
        handler: function (event) {
          ...
          switch (event.type) {
            ...
            case !dd.dragging && 'mousemove':
              ...
              if (Math.pow(event.pageX - dd.pageX, 2) + Math.pow(event.pageY
                      - dd.pageY, 2) < Math.pow(dd.distance, 2)) {
                break;
            ...
            default:
              ...
              if (dd.click === false && dd.dragging) {
                $.data(dd.mousedown, "suppress.click", new Date().getTime()
                    + 5);
              }
          ...
          do {
            if (ia = dd.interactions[i]) {
              ...
              $(elem || ia[mode] || dd.droppable).each(function (p, subject) {
                ...
                // handle the event
                result = subject ? $event.dispatch.call(subject, event,
                    callback)
                    : null;
        ...
        flatten: function (arr) {
          return $.map(arr, function (member) {
            return member && member.jquery ? $.makeArray(member) : member
            && member.length ? drag.flatten(member) : member;
          });
        },
