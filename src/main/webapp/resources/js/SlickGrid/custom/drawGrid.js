    ...
    var LineNumberFormatter = function(row, cell, value, columnDef, dataContext) {
        if (dataContext.parent != null){ // header row: a parent row is present
            if (dataContext.id == dataContext.parent) { // header row that is the parent row
                // return the appropriate item depending on the current state of the parent (expanded or collapsed)
...
function headerLineFilter(item, rows) {
    if (item.parent != null) {  // if it's a header line
        ...
        // weed out the non-parent header lines if the parent is collapsed (since they are not showing, we don't need to format them)
        ...
        // the parent line and the non-prent header lines will be formatted if the parent line isn't collapsed
...
function bindGridHeaderLineToggleEvent(grid, dataView, colNumber, headerLines, firstHeaderLine) {
    grid.onClick.subscribe(function(e, args) {
        if ($(e.target).hasClass("toggle")) {
            // get the event target (parent header line) and determine its state (expanded or collapsed)
