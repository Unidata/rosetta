...
function gridForVariableSpecification(grid, fileData, columns, rows, LineNumberFormatter, step) {
    ...
    $(function() {    
        // get the header line numbers from the session so we can identify the line type in the fileData array and format accordingly
        var headerLines = getFromSession("headerLineNumbers").split(/,/g);

        // denote which is the first or "parent" header line that will be shown when the rest of the header lines are collapsed
        ...
        for (var i = 0; i < fileData.length; i++) {  
            ...
            if (fileData[i] != "") { // sanity check to make sure the line isn't blank  
                ...
                // create a placeholder object to hold the line data, starting with the line number data for the first column
                ...
                if (jQuery.inArray(i.toString(), headerLines) < 0) { // it's not a header line
                    ...
                    if (bool == 1) { 
                        ...
                        for (var x = 0; x < colNumber; x++) { 
                            ...
                            // check session to see if variable input has already been entered by user 
                ...
                $("#dialog").dialog({
                    ...
                    buttons: {
                        ...
                        "cancel": function() { 
                            ...
                            // ugh!  Kludge to counter the fact the grid header button resets to previous options if revisiting dialog.
                            checkIfColumnIsDisabled(colNumber, grid);
...
function bindDialogEvents(sessionKey) {
    ...
    $("#dialog #variableNameTypeAssignment input[name=\"variableName\"]").focusout(function() {
        ...
        if (isCFStandardName($(this).attr("value"))) {
            // standard_name
            var metadataString = buildStringForSession(sessionKey + "Metadata", "standard_name", $(this).attr("value")); 
    ...
    $("#dialog #dataTypeAssignment input[name=\"dataType\"]").bind("click", function() {
        ...
        $("#dialog #dataTypeAssignment").find("label.error").text(""); // get rid of any error messages

        // concatenation the entered value to any existing Metadata values pulled from the session
        var metadataString = buildStringForSession(sessionKey + "Metadata", "dataType", $(this).attr("value"));
...
function bindGeneralMetadataEvents(sessionKey) {
    ...
    $("#dialog #additionalMetadataAssignment img#additionalMetadataChooser").unbind("click").bind("click", function() {
        ...
        if ($(this).attr("alt") == "Add Metadata") { 
            ...
            $("#additionalMetadataAssignment").find("label[for=\"additionalMetadataAssignment\"].error").text(""); // get rid of any global error messages
            ...
            if ($(additionalMetadataInputTags).length == 0) {
                ...
                if ($(additionalMetadataInputTags).find("li input[name=\"" + additionalMetadataSelected + "\"]").length > 0) {
                    $("#additionalMetadataAssignment").find("label[for=\"additionalMetadataAssignment\"].error").text("'" + getMetadataDisplayName(additionalMetadataSelected) + "' has already been selected.");
                } else {
                    // append new tag
        ...
        // Removing an additional metadata item         
            ...
            if ($(additionalMetadataInputTags).find("li input[name=\"" + additionalMetadataSelected + "'\"]").length == 0) {
                ...
                $("#additionalMetadataAssignment").find("label[for=\"additionalMetadataAssignment\"].error").text("'" + getMetadataDisplayName(additionalMetadataSelected) + "' has NOT been selected and therefore cannot be removed.");
            } else {
                ...
                removeItemFromSessionString(sessionKey + "Metadata", additionalMetadataSelected);
            }
        }
...
function bindAdditionalMetadataEvents(sessionKey) {
    ...
    $("#dialog #additionalMetadataAssignment ul li input").on("focusout", function() {
        ...
        $(this).parents("li").find("label[for=\"" + tagname + "\"].error").text(""); // get rid of any error messages
...
function bindUnitBuildEvents(sessionKey) {
    ...
    $("#dialog #unitBuilder img#unitBuilderChooser").bind("click", function() {
        ...
        if ($(this).attr("alt") == "Add To Units") { 
            ...
            // concatenation the entered value to any existing Metadata values pulled from the session
            var metadataString = buildStringForSession(sessionKey + "Metadata", "units", unitsInSession);
        ...
        // Removing from units 
            ...
            if (index >= 0) {
                ...
                // concatenation the entered value to any existing Metadata values pulled from the session
                var metadataString = buildStringForSession(sessionKey + "Metadata", "units", pre + post);

                // update the data in the session
                addToSession(sessionKey + "Metadata", metadataString); 

                // update units display in dialog to show new value
                $("#dialog #requiredMetadataAssignment input[name=\"units\"]").attr("value", pre + post);

            } else {
                $("#dialog #requiredMetadataAssignment #unitBuilder").find("label.error").text("'" + unitString + "' has NOT been detected in the current units and therefore cannot be removed.");
            }
        }
 ...
 *
 * @param sessionKey  The key used to store the data in the session.
 * @param tagName  The name attribute of the input tag, which happens to be the id of the metadata item as it appears in the reference XML file.
 ...
 * @param metadataNecessity  The importance of the metadata entry (required, recommended or additional)
 */
function populateMetadataInputTags(sessionKey, variableType, metadataNecessity) {
    ...
    for (var i = 0; i < metadata.length; i++) {                          
        ...
        // Grab the metadata entries that correspond to the variable type (coordinate or non-coordinate), 
        // grab the metadata entries that correspond to the variable necessity (required, recommended, or additional),
        ...
        // Grab the metadata entries that correspond to the both coordinate or non-coordinate variables
...
function populateDataFromSession(sessionKey) {
    ...
    if (variableValue) {  // the user has provided something for the variable name or opted not to use the column of data
        ...
        if (variableValue != "Do Not Use") { // variable name provided
            ...
            if (variableMetadataInSession) { 
                ...
                if (coordinateVariableSelected != null) {
                    ...
                    if (dataTypeSelected != null) {
                        // check the appropriate choice and update the metadata options accordingly
                        $("#dialog #dataTypeAssignment input[name=\"dataType\"][value=\"" + dataTypeSelected + "\"]").attr("checked", true);  
                       ...
                       // any required and recommended metadata stored in the session gets inserted when the metadata HTML is added to the dialog DOM
                       ...
                       for (var i = 0; i < metadataInSession.length; i++) {
                           if (isAdditionalMetadata(coordinateVariableSelected, metadataInSession[i])) {

                               var displayName = getMetadataDisplayName(metadataInSession[i]);

                               // see if the user has already provided the value to some of these metadata items.
                               var tagValue = getItemEntered(sessionKey + "Metadata", metadataInSession[i]);

                               var tag = createAdditionalMetadataTag(metadataInSession[i], displayName, tagValue);
            ...
            $("#dialog #variableNameTypeAssignment input[name=\"variableNameType\"][value=\"do_not_use\"]").attr("checked", true);
        ...
        if (coordinateVariableInSession != null) {
            ...
            if (dataTypeInSession != null) {
                ...
                if (metadataProvided.length > 0) {
                    // get the metadata names (not values) held in the session
                    var metadataInSession = getKeysFromSessionData(metadataProvided);
                    var requiredMetadata = getKnownRequiredMetadataList(coordinateVariableInSession);
